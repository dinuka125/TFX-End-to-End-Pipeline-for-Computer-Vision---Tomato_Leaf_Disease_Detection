...
pipeline_name = 'tomato_pipeline'

data_dir =  os.path.join(pipeline_dir, 'data', 'tomato_data')
...
pipeline_root = os.path.join(output_base, 'pipeline_root/2')
...
def init_components(data_root, module_file, serving_model_dir,
  ...
  evaluator = Evaluator(examples=transform.outputs['transformed_examples'],model=trainer.outputs['model'],baseline_model=model_resolver.outputs['model'],eval_config=eval_config)
